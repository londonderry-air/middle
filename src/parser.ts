import { Token, TokenizeSource, Attribute } from "./types";
import { genTextToken, genParagraphToken } from "./token";
import { isBlockquoteMatch, isListMatch, isPreMatch } from "./lexer";
import {
  BLOCKQUOTE_ELM_REGXP,
  PARAGRAPH_NOT_COVERED_ELM_REGXPS,
  TEXT_ELM_REGXPS,
} from "./regex";
...
const tokenizeText = (textElement: string, initialRoot: Token = rootToken) => {
  const elements: Token[] = [];
  let parent: Token = initialRoot;

  const isParagraphCoverdLine =
    PARAGRAPH_NOT_COVERED_ELM_REGXPS.map((item) => ({
      element: item.element,
      matchList: textElement.match(item.regexp) as RegExpMatchArray,
    })).filter((item) => item.matchList).length === 0 && // pタグで囲まない要素がある場合は、ループ処理内でそれぞれ pタグ の判定をつける
  ...
  const _tokenize = (originalText: string, p: Token) => {
    ...
    while (isContinueTokenize(analysingText)) {
      const matchList: TokenizeSource[] = TEXT_ELM_REGXPS.map((item) => ({
        element: item.element,
      ...
      if (isNoPrefixFound) {
        ...
        const isSetParagraphParentToken =
          mostOuterToken.element !== "h1" &&
        ...
        const isTextExistBeforePrefix =
          (mostOuterToken.matchList.index ?? -1) > 0;
        ...
        if (isTextExistBeforePrefix) {
          const text = analysingText.substring(
            0,
            mostOuterToken.matchList.index
          );
          const token = genTextToken(getRandomStr(), text, parent);
          elements.push(token);
        ...
        if (parent.elmType === "code") {
          const token = genTextToken(
            getRandomStr(),
            mostOuterToken.matchList[0],
            parent
          );
          elements.push(token);

          analysingText = analysingText.replace(
            mostOuterToken.matchList[0],
            ""
          );
        } else {
...
export const tokenizeBlockquote = (md: string) => {
  ...
  const tokens: Token[] = [];
  const bqTokens: Token[] = []; // ネストの深さを配列の長さで管理する
